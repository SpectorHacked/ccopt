# effigent — The Agent Waste Report

Graph-based determinism detection for AI agents. effigent maps every run your Claude Code agents make, proves which runs are the same procedure repeated, and shows you — in dollars — what to cache, compile, downsize, or fix.

Implements the MVP scoped in the internal `effigent-mvp-spec.md` (kept out of the public repo).

## How it works

1. **Capture** — Claude Code already writes a complete JSONL transcript per session to `~/.claude/projects/` including per-message token usage. effigent parses those directly (zero-install). For programmatic/CI runs, `effigent run --agent <id> -- claude -p "..."` tags the session with a logical agent identity.
...
| `packages/cli` | `effigent analyze` (local-only mode), `effigent sync` (upload), `effigent run` (CI wrapper) |
...
effigent ships as a single self-contained file — the only prerequisite is Node 20+. No npm install, no dependencies, no changes to your agents.

```bash
# one-liner (or run ./install.sh from a checkout):
curl -fsSL https://effigent.ai/install.sh | sh

effigent doctor     # verifies capture, attribution, auth mode, and server reachability
effigent analyze    # your first Waste Report, from transcripts you already have
```

`effigent doctor` is the out-of-the-box check: it finds your existing Claude Code / Agent SDK transcripts (they're already being written — capture requires nothing), confirms they parse, and tells you exactly what's missing for isolated or hosted mode.

### Onboarding a teammate: one command

Whoever owns the workspace runs `effigent invite [--agent <filter>]` and sends the printed line. The teammate pastes it:

```bash
curl -fsSL https://effigent.ai/install.sh | sh -s -- --join <token>
```

That single command installs effigent, saves the workspace server + key + attribution rules, schedules a recurring sync (launchd on macOS, cron on Linux — every 15 minutes, filtered so unrelated local sessions stay private), verifies the API key, and uploads their existing history. The token embeds the workspace API key — share it privately.
...
Rebuild the distributed file with `npm run bundle` (emits `dist/effigent.cjs`, committed so the Action and raw-URL install work).
...
cd packages/server
EFFIGENT_ADMIN_TOKEN=dev-admin-token node dist/index.js   # migrates + listens on :8787
...
# customer machine
effigent sync --server http://localhost:8787 --key cck_…
curl -X POST localhost:8787/api/v1/analyze -H 'authorization: Bearer cck_…'
# → { reportUrl: "http://localhost:8787/r/<uuid>" }
```

Server env: `DATABASE_URL`, `EFFIGENT_ADMIN_TOKEN` (required), `EFFIGENT_DATA_DIR` (blob root, default `./data`), `PORT` (8787), `EFFIGENT_PUBLIC_BASE_URL`. **Everything is trigger-only** — there are no scheduled jobs; reports and AI analyses are generated exactly when requested.
...
|---|---|---|
| `EFFIGENT_LLM_PROVIDER` | `anthropic` | `openai-compatible` |
| `EFFIGENT_LLM_MODEL` | `claude-opus-4-8` (default) | e.g. `google/gemini-3.5-flash` (OpenRouter), `gpt-4o`, a local Ollama model |
| `EFFIGENT_LLM_BASE_URL` | — | e.g. `https://openrouter.ai/api/v1`, `http://localhost:11434/v1` |
| `EFFIGENT_LLM_API_KEY` | falls back to `ANTHROPIC_API_KEY` | falls back to `OPENAI_API_KEY` |
...
- **Disk** (default): `EFFIGENT_DATA_DIR` — dev and single-box installs.
- **S3-compatible** (set `EFFIGENT_S3_BUCKET` + `EFFIGENT_S3_ENDPOINT` + `EFFIGENT_S3_ACCESS_KEY_ID` + `EFFIGENT_S3_SECRET_ACCESS_KEY` [+ `EFFIGENT_S3_REGION`, default `auto`]): works with Cloudflare R2, Backblaze B2, AWS S3, and MinIO (integration-verified against MinIO).
...
Deploy: create the Neon DB and R2 bucket (+ API token), push this repo, point Render at it (`render.yaml`), paste `DATABASE_URL` (with `?sslmode=require`) and the `EFFIGENT_S3_*` values, set `EFFIGENT_PUBLIC_BASE_URL` to the Render URL. Migrations run automatically on boot. Then `effigent login --server https://<app>.onrender.com --key …` and every `effigent invite` from that point embeds the stable public URL.
...
effigent never requires code changes in the agent it observes. Anything built on Claude Code or the Claude Agent SDK already writes complete session transcripts to `~/.claude/projects/`; effigent attributes them three ways:
...
effigent run --agent dev-teammate:acme/api -- pnpm --filter @moonshot/cli exec tsx src/run.ts --repo acme/api --requirement "…"
effigent run --agent nightly-feed-sync -- claude -p "Sync the product feed"

# 2. Tag existing sessions after the fact:
effigent tag --agent dev-teammate:acme/api <session-id> …
...
jobs:
  dev-teammate:
    ...
    steps:
      ...
      - name: Run dev-teammate under effigent
        env:
          ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
          EFFIGENT_SERVER: https://effigent.yourcompany.com
          EFFIGENT_API_KEY: ${{ secrets.EFFIGENT_API_KEY }}
        run: |
          npx effigent run --agent "dev-teammate:${{ inputs.repo }}" --isolated -- \
